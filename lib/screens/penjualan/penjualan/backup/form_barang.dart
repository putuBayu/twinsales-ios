...
class _TambahBarangState extends State<FormBarang> {
  ...
  var tipeHarga = '', dataBarang = [], dataPromo = []; int idStock = 0, idPromo = 0, idHarga = 0, lQty = 0, lQtyPcs = 2, indexBarang; // index barang untuk menghapus array barang
  bool loaderBarang = true, loaderPromo = true, isSave = false;

  @override
  void initState(){
    super.initState(); initForms();
  }
  ...
  void saveBarang() async {
    ...
    if( idStock == 0 || qty.text == '' && qtyPcs.text == '' || qty.text == '0' && qtyPcs.text == '0' ){
      ...
      if(widget.formData == null){
        ...
        Api.put('detail_penjualan/'+widget.formData['id'].toString(), formData: data, then: (s, data){
          Map res = decode(data);
  ...
  Widget build(BuildContext context) {
    return Unfocus(
      child: Scaffold(
        appBar: Wi.appBar(context, title: widget.formData == null ? 'Tambah Barang' : 'Edit Barang'),
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                ...
                FormControl.select(context, label: 'Pilih Barang', enabled: widget.formData != null ? false : true, controller: namaBarang, onTap: (){
                ...
                FormControl.select(context, label: 'Pilih Promo', mb: namaPromo.text.isEmpty ? 25 : 0, controller: namaPromo, onTap: (){
                ...
                namaPromo.text.isEmpty ? SizedBox.shrink() : Container(
...
class _ListBarangState extends State<ListBarang> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Scaffold(
        appBar: Wi.appBar(context, title: FormControl.search(hint: 'Ketik kode atau nama barang', autofocus: true, onChange: (String s){
          var k = s.toLowerCase();
          setState((){
        ...
        body: loading ? Wi.spiner(size: 50) :
         new Container(
          color: Colors.white,
          child: this.dataFiltered == null || this.dataFiltered.length == 0 ? Wi.noData(message: 'Tidak ada data barang') :
...
class _ListPromoState extends State<ListPromo> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: Wi.appBar(context, title: FormControl.search(hint: 'Ketik nama promo', autofocus: true, onChange: (String s){
        var k = s.toLowerCase();
        setState((){
      ...
      body: loading ? Wi.spiner(size: 50) : new Container(
        color: Colors.white,
        child: 
