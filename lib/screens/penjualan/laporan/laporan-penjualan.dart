...
class _LaporanPenjualanState extends State<LaporanPenjualan> {
  ...
  var dataPenjualan = [], loaded = true, startDate = Dt.ymd, endDate = Dt.ymd,
      totalQty = 0, totalPcs = 0, totalEc = 0, totalVal = 0, totalQo = 0,
      totalPo = 0, totalVo = 0;
  DateTime initFirstDate = DateTime.now(), initEndDate = DateTime.now();

  var expandFooter = false, isSales = false;
  var tEc = 0, tSku = 0, tQo = 0, tPo = 0, tQty = 0, tPcs = 0, tVo = 0, tV = 0,
      tTni = 0, tBg = 0, tTrf = 0, tRtr = 0, tLn = 0, tTgt = 0, tAch = 0, avgAch = 0;

  var tJin = 0, tCsh = 0, tCrd = 0;
  ...
  var labelsAdmin = [
    ...
    'Total Target',
    'Total Achievement',
    'Total Pelunasan Tunai',
    'Total Pelunasan Bg',
    'Total Pelunasan Transfer',
    'Total Pelunasan Retur',
    'Total Pelunasan Lainnya'
  ];

  getDataPenjualan() async {
    var prefs = await SharedPreferences.getInstance(), roles = prefs.getString('roles');
    setState(() { loaded = false; totalQty = totalPcs = totalEc = totalVal = totalQo = totalPo = totalVo = 0; });
    ...
    if(isSales){
      // url = 'report/laporan_penjualan?id_salesman='+id.toString()+'&start_date='+startDate+'&end_date='+endDate;
      url = 'report/laporan_penjualan?id_salesman='+id.toString()+'&date='+startDate;
    }else{
      // url = 'report/laporan_penjualan?id_salesman=all&date='+startDate+'&end_date='+endDate;
      url = 'report/laporan_penjualan?id_salesman=all&date='+startDate;
    }
  ...
  @override
  void initState() {
    super.initState(); getDataPenjualan();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: Wh.appBar(context, title: 'Laporan Penjualan',
      ...
      body: !loaded ? ListSkeleton(length: 10)
          : dataPenjualan == null || dataPenjualan.length == 0
                        ...
                        Expanded(
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, i){
                              ...
                              var pt = data['penagihan_tunai'] == null ? 0 : 'Rp '+ribuan(data['penagihan_tunai'].toString());
                              var pbg = data['penagihan_bg'] == null ? 0 : 'Rp '+ribuan(data['penagihan_bg'].toString());
                              var ptf = data['penagihan_transfer'] == null ? 0 : 'Rp '+ribuan(data['penagihan_transfer'].toString());
                              var prt = data['penagihan_retur'] == null ? 0 : 'Rp '+ribuan(data['penagihan_retur'].toString());
                              var pnl = data['penagihan_lainnya'] == null ? 0 : 'Rp '+ribuan(data['penagihan_lainnya'].toString());
                      ...
                      children: [
                        AnimatedContainer(
                          ...
                          child: ClipRRect(
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: List.generate(labelsAdmin.length, (int i){
                                  var values = [
                                    tJin,
                                    tEc,
                                    tSku,
                        ...
                        Positioned(
                          ...
                          child: WidSplash(
                            ...
                            child: Icon(expandFooter ? Ic.chevron() : Ic.chevronUp(), size: 20, color: Colors.blueGrey,),
                          ),
                        )
                      ]
                    )
