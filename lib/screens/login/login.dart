...
class _LoginState extends State<Login> {
  ...
  initScreen() async {
    ...
    FlutterStatusbarcolor.setStatusBarWhiteForeground(
        useWhiteForeground(Colors.transparent) ? true : false);
      ...
      if (user != null) {
        setState(() {
          if (prefs.getBool('loginWithEmail') != null) {
            email.text = prefs.getBool('loginWithEmail')
                ? decode(user)['email']
                : decode(user)['phone'].toString();
          }
        });
      }
    }
  ...
  signin() async {
    request() {
      var formData = emailValidate(email.text)
          ? {
              // jika bukan format email -> no. telepon
              'email': email.text,
      ...
      if (email.text.isEmpty || password.text.isEmpty) {
        ...
        timer = Timer(Duration(seconds: 15), () {
          ...
          Wh.alert(context,
              ...
              message:
                  'Tidak dapat terhubung ke server, coba periksa koneksi internet anda atau beritahu TIM IT perihal masalah ini.',
              textConfirm: 'Hubungi TIM IT', onTap: () {
        ...
        Request.post('login',
          ...
          if (roles.indexOf('salesman') < 0 &&
              roles.indexOf('salesman canvass') < 0 &&
              roles.indexOf('administrator') < 0 &&
              roles.indexOf('sales supervisor') < 0 &&
              roles.indexOf('sales koordinator') < 0) {
            Wh.alert(context,
                message:
                    'Akun ini tidak terdaftar untuk aplikasi sales, pastikan Anda login dengan akun sales.',
                icon: Ic.userx(),
                color: TColor.red(),
                borderColor: TColor.red());
          } else {
            ...
            setPrefs(
                'loginWithEmail', formData['email'] != null ? true : false);
            Wh.dialog(context,
                child: PreparingData(
                  token: token,
                  roles: roles,
              ...
              child: Container(
                  ...
                  child: Column(
                      ...
                      children: [
                        ColorFiltered(
                          ...
                          child: Container(
                            ...
                            margin: EdgeInsets.only(
                                bottom: 25,
                                top: Mquery.statusBar(context) + 30),
                        ...
                        Fc.textfield(
                          controller: email,
                          hint: 'Inputkan alamat email',
                          length: 50,
                          suffix: Icon(Ic.mail(), size: 18),
                          node: emailNode,
                          submit: (String s) {
                              ...
                              WidSplash(
                                ...
                                onTap: () {
                                  Wh.alert(context,
                                      ...
                                      message:
                                          'Untuk permasalahan seperti lupa password akun atau kendala lainnya yang berhubungan dengan aplikasi ini silahkan hubungi TIM IT.',
                                      textConfirm: 'Hubungi TIM IT', onTap: () {
                                    goto(
                                        'https://chat.whatsapp.com/BF6SzDk0VhCJqtwgV4YAbI');
                                  });
                                },
                              ),
                            ])
...
class _PreparingDataState extends State<PreparingData> {
  ...
  isSales() {
    if (widget.roles.indexOf('salesman') > -1 ||
        widget.roles.indexOf('salesman canvass') > -1) {
      return true;
    }
  ...
  Future getDataToko() async {
    if (!isSales()) {
      ...
      Request.get('penjualan/list/toko', then: (status, res) {
        ...
        removePrefs(list: [
          'token'
        ]); // hapus token jadi saat app dibuka lagi user masih di halaman login
  ...
  Future getDataUser() async {
    ...
    Request.get(
        'salesman/get/salesman_principal?id_user=' +
            widget.user['id'].toString(), then: (status, res) {
  ...
  Widget build(BuildContext context) {
    return SlideUp(
      ...
      decoration: BoxDecoration(
          image: DecorationImage(
              image: AssetImage('assets/img/line-card.png'), fit: BoxFit.fill)),
          ...
          : Row(
              children: <Widget>[
                ...
                Flexible(
                  ...
                  child:
                      text('Mohon menunggu, sedang menyiapkan data ' + onRun),
                ))
              ],
            ),
    ));
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              text(
                  'Aplikasi yang Anda gunakan adalah versi yang lama, untuk menghindari terjadinya kesalahan silahkan update ke versi yang terbaru (' +
                      widget.version +
                      '). \n\nDownload di playstore atau hubungi Tim IT.',
