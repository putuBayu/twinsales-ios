...
class Request {
        ...
        try {
          http.get(api(url), headers: !authorization ? {} : {
            ...
            if(res.statusCode != 200){
              var response = {'status': res.statusCode, 'body': res.statusCode == 500 || res.statusCode == 401 ? {} : decode(res.body)};
              error(response);
            }else{
  ...
  static put(url, {formData, debug: false, authorization: true, Function then, Function error}) async{
    var prefs = await SharedPreferences.getInstance();
  ...
  switch (status) {
    ...
    default: //Wh.toast(status.toString()+' - Unknown Error'); print(response['body']);
      response['body'].forEach((k, v){
        Wh.toast(response['body'][k] is List ? response['body'][k].join('') : response['body']['error']);
